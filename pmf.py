...
class PMF(object):
    ...
    def train(self, users, movies, ratings, alpha, lambda_u, lambda_v,
        ...
        for i in range(num_iterations):
            ...
            dUV=2*(np.sum(self.U[:,users_batch]*self.V[:,movies_batch],axis=0)-ratings_batch).reshape([-1,1]) #derivative of Transpose(U)V
            
            dU=self.V[:,movies_batch].T*dUV #derivative of user

            dV=self.U[:,users_batch].T*dUV #derivative of movie
            
            for i, user in enumerate(users_batch): #updating user_batch
                self.U[:,user]=self.U[:,user]-alpha*(dU[i]+lambda_u*self.U[:,user])
            
            for i, movie in enumerate(movies_batch): #updating movie_batch
